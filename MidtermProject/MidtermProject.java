...
public class MidtermProject {
    ...
    // NewDeck: Wipes all old decks along with scores and create a new deck;
    // includes face, suit, and number
    ...
    // Done by reading the string at given index and adding corresponding value of
    // it to total score using if statments
    ...
    public static int CalculateScore(ArrayList<String> GivenDeck) {
        ...
        for (int i = 0; i < GivenDeck.size(); i++) {
            if (GivenDeck.get(i).contains("Ace")) {
                Score += 1;
                Ace = true;
            } else if (GivenDeck.get(i).contains("10") || GivenDeck.get(i).contains("Jack")
                    || GivenDeck.get(i).contains("Queen") || GivenDeck.get(i).contains("King")) {
                Score += 10;
                Ten = true;
            } else {
                for (int j = 2; j <= 9; j++) {
        ...
        // Blackjack can only trigger on initial hand, checked by GivenDeck size only
        // being 2 cards and if an Ace and Ten are presen
        if (Ace == true && Ten == true && GivenDeck.size() == 2)
            return -1;
        return Score;
    }

    // DealCardX: Chooses a random element from ArrayList Deck and adds it to player
    // or computer deck, repsectively
    // also removes card from deck so it isn't dealt to both the player and computer
    // during next hand
    // Condition used to determine amonunt of cards dealt. Ran twice initally to
    // mimic initial deal, then only runs once until round ends
    ...
    // Also displays current hand, current score, as well as prevents the Player
    // from hiiting if they scored a Blackjack or Busted
    ...
    // ComputerMove: Functions the same as NextMove, but for computer and doesn't
    // return
    ...
    // WinLogic: If player does not bust, checks for a ties, who's closer to 21, or
    // if computer scored Blackjack
